...
```mermaid
graph LR
    subgraph Local_Workspace [Local Environment WSL2]
        A[index.html] -->|Packaged By| B(Dockerfile)
        B -->|Local Test| C(Docker Engine)
    end

    subgraph Version_Control [GitHub Cloud]
        D[git push] -->|Triggers| E{GitHub Actions CI}
    end

    subgraph Automated_Runner [Cloud Virtual Machine]
    ...
    C -.->|Synced Via| D
    Local_Workspace -->|Code Upload| Version_Control
    Version_Control -->|Pipeline Execution| Automated_Runner
